...
@router.get("/student")
def student(db: Session=Depends(get_db), user=Depends(require_role("student"))):
...
@router.get("/industry")
def industry(db: Session=Depends(get_db), user=Depends(require_role("company"))):
...
@router.get("/mentor")
def mentor(db: Session=Depends(get_db), user=Depends(require_role("MENTOR_TRAINER"))):
...
def institution(organization_id: str, db: Session=Depends(get_db), user=Depends(require_role("institution"))):
