...
class User(Base):
    __tablename__='users'; __table_args__=(CheckConstraint("role IN ('STUDENT','INDUSTRY','INSTITUTION','ACADEMICIAN','ADMIN','FACULTY','INSTITUTION_ADMIN','MENTOR_TRAINER','INDUSTRY_MEMBER_RECRUITER','INDUSTRY_ADMIN')",name='ck_user_role'),)
    id: Mapped[uuid.UUID]=mapped_column(primary_key=True,default=uuid.uuid4)
    email: Mapped[str]=mapped_column(String(255),unique=True,index=True)
    password_hash: Mapped[str]=mapped_column(String(255)); role: Mapped[str]=mapped_column(String(40),default='STUDENT')
